...
function collectSettingsPayload() {
  ...
  return {
    ...
    gmailCodeApiAuthToken: inputGmailCodeAuthToken?.value || '',
...
function updateMailProviderUI() {
  ...
  if (gmailCodeSection) {
    gmailCodeSection.style.display = useGmailCode ? '' : 'none';
  }
  if (luckmailSection) {
  ...
  if (autoHintText) {
    autoHintText.textContent = useHotmail
      ...
      : (useLuckmail
        ...
        : (useGmailCode
          ? '步骤 3 会自动请求 GmailCode 别名邮箱并用于收码'
      : (useGeneratedAlias
        ? '步骤 3 会自动生成邮箱，无需手动获取'
