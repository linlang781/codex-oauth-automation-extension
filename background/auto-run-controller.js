        ...
        while (attemptRun <= maxAttemptsForRound) {
          ...
          try {
            ...
            if (blockedBySignupUserAlreadyExists) {
              ...
              await broadcastStopToContentScripts();
              if (typeof isGmailCodeProvider === 'function' && typeof markGmailCodeAliasUsed === 'function') {
                const stateForMark = await getState();
                if (isGmailCodeProvider(stateForMark)) {
                  const alias = String(stateForMark.email || '').trim();
                  if (alias) {
                    await markGmailCodeAliasUsed(stateForMark, alias);
                    await addLog(`GmailCode 别名 ${alias} 因 user_already_exists 已通过 API 标记为已使用。`, 'ok');
                  }
                }
              }
              if (!autoRunSkipFailures) {
